...
class King extends ChessPiece {
    ...
    // Return true if kingside castling is legal. False otherwise (does not check for checks)
    ...
    boolean isLegal(ChessPiece[][] board, int sRow, int sCol, int eRow, int eCol) throws ChessException {
        ...
        boolean debugA = legalKingsideCastling(board, sRow, sCol, eRow, eCol) ;
        boolean debugB = legalQueensideCastling(board, sRow, sCol, eRow, eCol);
        boolean debugC = (Math.abs(eRow - sRow) <= 1 && Math.abs(eCol - sCol) <= 1);
